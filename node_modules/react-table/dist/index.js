...
var useTable = function useTable(props) {
  ...
  if (process.env.NODE_ENV !== 'production' && debug) console.time('hooks.useInstanceBeforeDimensions');
  instanceRef.current = applyHooks(instanceRef.current.hooks.useInstanceBeforeDimensions, instanceRef.current);
  if (process.env.NODE_ENV !== 'production' && debug) console.timeEnd('hooks.useInstanceBeforeDimensions');
  calculateDimensions(instanceRef.current);
  if (process.env.NODE_ENV !== 'production' && debug) console.time('hooks.useInstance');
  instanceRef.current = applyHooks(instanceRef.current.hooks.useInstance, instanceRef.current);
  if (process.env.NODE_ENV !== 'production' && debug) console.timeEnd('hooks.useInstance'); // Each materialized header needs to be assigned a render function and other
...
var useInstance$8 = function useInstance(instance) {
